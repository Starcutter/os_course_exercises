...
2. 不同特权级有什么区别？
	...
	**答**：某些指令只能在较高的特权级运行，如特权指令只能在ring 0下运行；某些数据只能在较高的特权级访问。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

	**答**：
	
	- 中断响应：根据中断向量访问中断门时，需要CPL <= DPL[门]，然后根据中断门存储的段选择子访问中断服务例程所在段时，需要CPL >= DPL[段]
	- 函数调用：执行跳转，需要CPL <= 函数程序段的DPL
	- 指令执行：若访问数据，需要RPL <= 数据段的DPL
...
1. 一条指令在执行时会有哪些可能的特权级判断？
	...
	**答**：
	...
	- 访问数据：判断RPL <= 数据段DPL
	- 执行跳转：判断CPL <= 程序段DPL
	- 产生中断：判断CPL <= DPL[门] && CPL >= DPL[段]

2. 在什么情况下会出现特权级切换？

	**答**：在用户态产生中断、异常、系统调用时，从用户态切换到内核态；从中断服务例程返回时，根据栈中存储的特权级信息，可能从内核态返回用户态。

3. int指令在ring0和ring3的执行行为有什么不同？

	**答**：ring 3执行int指令，需要切换到ring 0的栈，而且压栈的内容多出了SS和ESP两项。ring 0则没有堆栈切换，压栈内容没有SS和ESP.

4. 如何利用int和iret指令完成不同特权级的切换？

	**答**：调用int指令压栈后，修改栈中的内容，再调用iret指令，栈中被修改过的CPL、CS、EIP等内容被获取，就能完成切换。

5. TSS和Task Register的作用是什么？

	**答**：TSS保存了0-2特权级的栈信息，如SS和ESP等。Task Register保存了用来在GDT中寻找TSS Descriptor的段选择子，同时也缓存了从TSS Descriptor中获取的TSS基地址和段长。
...
1. 一条指令执行时最多会出现多少次地址转换？
	...
	**答**：逻辑地址经段机制到线性地址，线性地址经页机制再到物理地址，总共两次。

2. 描述X86-32的MMU地址转换过程；

	**答**：取逻辑地址中的段选择子，经过GDT查询，得到段基地址，加上偏移量，得到线性地址。取线性地址的页号，先在TLB中查询，如果命中，取TLB中对应的帧号加上偏移量，得到物理地址；否则，查询页表，可能有多级页表，逐级查询后得到帧号，加上偏移量得到物理地址。
